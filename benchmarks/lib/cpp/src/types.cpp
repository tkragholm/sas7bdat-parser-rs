...
std::string to_string(DATETIME _x) {
  ...
  const auto year = static_cast<int>(d.year());
  const auto month = static_cast<unsigned int>(d.month());
  const auto day = static_cast<unsigned int>(d.day());
  if (t.fractional_seconds()) {
    auto r = fmt::format_to_n(szBuffer, sizeof(szBuffer),
                              "{:04}-{:02}-{:02} {:02}:{:02}:{:02}.{:06}",
                              year, month, day, t.hours(), t.minutes(),
                              t.seconds(), t.fractional_seconds());
    size = r.size;
  } else {
    auto r = fmt::format_to_n(
        szBuffer, sizeof(szBuffer), "{:04}-{:02}-{:02} {:02}:{:02}:{:02}",
        year, month, day, t.hours(), t.minutes(), t.seconds());
    size = r.size;
  }
...
std::string to_string(DATE _x) {
  char szBuffer[16];
  size_t size{0};
  const auto year = static_cast<int>(_x.year());
  const auto month = static_cast<unsigned int>(_x.month());
  const auto day = static_cast<unsigned int>(_x.day());
