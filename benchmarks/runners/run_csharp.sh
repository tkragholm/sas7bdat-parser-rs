...
PROJECT_DIR="${ROOT}/benchmarks/csharp/SasBenchmarks"
...
CONFIG_FILE="${ROOT}/benchmarks/csharp/NuGet.Config"
BUILD_ROOT="${ROOT}/benchmarks/.build/csharp"
RESTORE_LOG="${BUILD_ROOT}/dotnet-restore.log"
BUILD_LOG="${BUILD_ROOT}/dotnet-build.log"
realpath_py() {
...
if [[ "${needs_restore}" == true ]]; then
  ...
  if ! run_dotnet "${restore_args[@]}" >"${RESTORE_LOG}" 2>&1; then
    cat "${RESTORE_LOG}" >&2
    exit 1
  fi
fi
...
if [[ "${needs_build}" == true ]]; then
  if ! run_dotnet build --nologo --verbosity quiet --no-restore "${PROJECT_FILE}" \
    -c "${CONFIGURATION}" >"${BUILD_LOG}" 2>&1; then
    cat "${BUILD_LOG}" >&2
    exit 1
  fi
fi
...
if [[ "${BUILD_ONLY}" == false ]]; then
  if [[ -n "${CSV_OUTPUT}" ]]; then
    run_dotnet exec "${OUTPUT_DLL}" --csv "${CSV_OUTPUT}" "${FILE}"
  else
