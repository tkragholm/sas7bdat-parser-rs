...
fn verify_fixture(path: &Path) -> Result<()> {
    ...
    match load_reference_snapshot("pandas", &absolute) {
        Ok(Some(reference_snapshot)) => {
            compare_snapshots("pandas", &absolute, &snapshot, &reference_snapshot);
        }
        Ok(None) => {
            panic!(
                "missing pandas reference snapshot for {} (expected at {})",
                normalized_relative_path(&absolute),
                reference_snapshot_path("pandas", &absolute).display()
            );
        }
        Err(err) => {
            panic!(
                "failed to load pandas reference snapshot for {}: {err}",
                normalized_relative_path(&absolute)
            );
        }
    }
