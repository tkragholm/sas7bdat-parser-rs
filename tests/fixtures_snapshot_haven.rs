...
fn verify_fixture(path: &Path) -> Result<()> {
    ...
    match load_reference_snapshot("haven", &absolute) {
        Ok(Some(reference_snapshot)) => {
            compare_snapshots("haven", &absolute, &snapshot, &reference_snapshot);
        }
        Ok(None) => {
            panic!(
                "missing haven reference snapshot for {} (expected at {})",
                normalized_relative_path(&absolute),
                reference_snapshot_path("haven", &absolute).display()
            );
        }
        Err(err) => {
            panic!(
                "failed to load haven reference snapshot for {}: {err}",
                normalized_relative_path(&absolute)
            );
        }
    }
