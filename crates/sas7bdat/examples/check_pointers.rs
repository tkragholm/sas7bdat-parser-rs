    ...
    for page_index in 0..header.page_count {
        ...
        for pointer_index in 0..subheader_count {
            ...
            let (offset, length) = if header.uses_u64 {
                let offset_raw = read_u64(header.endianness, &pointer[0..8]);
                let length_raw = read_u64(header.endianness, &pointer[8..16]);
                let Ok(offset) = usize::try_from(offset_raw) else {
                    println!(
                        "page {page_index} pointer {pointer_index}: offset {offset_raw} exceeds usize"
                    );
                    continue;
                };
                let Ok(length) = usize::try_from(length_raw) else {
                    println!(
                        "page {page_index} pointer {pointer_index}: length {length_raw} exceeds usize"
                    );
                    continue;
                };
                (offset, length)
