...
use sas7bdat_test_support::{
    ...
    fixtures_snapshot_util::{
        absolute_path, collect_cpp_snapshot, collect_snapshot, cpp_available, should_skip,
    },
...
static CPP_REPORT: OnceLock<Mutex<CppReport>> = OnceLock::new();

fn report() -> &'static Mutex<CppReport> {
    CPP_REPORT.get_or_init(|| Mutex::new(CppReport::new()))
}
...
impl Drop for ReportGuard {
    fn drop(&mut self) {
        ...
        if !report.unsupported_charset.is_empty() || !report.mismatches.is_empty() {
            eprintln!("cpp comparison summary:");
        ...
        if let Some(path) = report_path("SAS7BDAT_CPP_REPORT", "cpp.json", "SAS7BDAT_VERIFY_CPP") {
            let payload = serde_json::json!({
                "unsupported_charset": report.unsupported_charset,
                "mismatches": report.mismatches,
            });
            if let Err(err) = write_report(path, &payload) {
                eprintln!("failed to write cpp report: {err}");
            }
        }
    }
}
...
fn verify_fixture(path: &Path) -> Result<()> {
    let _ = &_REPORT_GUARD;
    if std::env::var_os("SAS7BDAT_VERIFY_CPP").is_none() {
        return Ok(());
    }
    ...
    let Some(reference_snapshot) = collect_cpp_snapshot(&absolute) else {
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        compare_snapshots("cpp", &absolute, &snapshot, &reference_snapshot);
    }));
    if let Err(payload) = result {
        ...
        if std::env::var_os("SAS7BDAT_CPP_FAIL_ON_MISMATCH").is_some() {
