...
static READSTAT_REPORT: OnceLock<Mutex<ReadstatReport>> = OnceLock::new();

fn report() -> &'static Mutex<ReadstatReport> {
    READSTAT_REPORT.get_or_init(|| Mutex::new(ReadstatReport::new()))
}
...
impl Drop for ReportGuard {
    fn drop(&mut self) {
        ...
        if !report.unsupported_charset.is_empty() || !report.mismatches.is_empty() {
            eprintln!("readstat comparison summary:");
        ...
        if let Some(path) = report_path(
            "SAS7BDAT_READSTAT_REPORT",
            "readstat.json",
            "SAS7BDAT_VERIFY_READSTAT",
        ) {
            ...
            if let Err(err) = write_report(path, &payload) {
                eprintln!("failed to write readstat report: {err}");
            }
        }
    }
}
...
fn verify_fixture(path: &Path) -> Result<()> {
    let _ = &_REPORT_GUARD;
    if std::env::var_os("SAS7BDAT_VERIFY_READSTAT").is_none() {
        return Ok(());
    }
    ...
    let Some(reference_snapshot) = collect_readstat_snapshot(&absolute) else {
    ...
    if let Err(payload) = result {
        ...
        if std::env::var_os("SAS7BDAT_READSTAT_FAIL_ON_MISMATCH").is_some() {
