#![allow(dead_code, clippy::pedantic)]
use crate::{
    common,
    reference::{Snapshot, relative_to_manifest},
};
use csv::ReaderBuilder;
use sas7bdat::{
    SasReader,
    parser::{ColumnKind, NumericKind, parse_metadata},
};
use serde_json::json;
use std::{
    fs,
    path::{Path, PathBuf},
    process::Command,
    sync::{
        OnceLock,
        atomic::{AtomicUsize, Ordering},
    },
};

pub const SKIP_FIXTURES: &[&str] = &[
    "fixtures/raw_data/pandas/corrupt.sas7bdat",
    "fixtures/raw_data/pandas/zero_variables.sas7bdat",
    "fixtures/raw_data/csharp/54-class.sas7bdat",
    "fixtures/raw_data/csharp/54-cookie.sas7bdat",
    "fixtures/raw_data/csharp/charset_zpce.sas7bdat",
    "fixtures/raw_data/csharp/date_format_dtdate.sas7bdat",
    "fixtures/raw_data/csharp/date_formats.sas7bdat",
    "fixtures/raw_data/ahs2013/topical.sas7bdat",
];

pub fn should_skip(path: &Path) -> bool {
    let normalized = relative_to_manifest(path)
        .iter()
        .map(|component| component.to_string_lossy())
        .collect::<Vec<_>>()
        .join("/");
    SKIP_FIXTURES.contains(&normalized.as_str())
}

pub fn absolute_path(path: &Path) -> PathBuf {
    if path.is_absolute() {
        path.canonicalize().unwrap_or_else(|_| path.to_path_buf())
    } else {
        let joined = common::repo_root().join(path);
        joined.canonicalize().unwrap_or(joined)
    }
}

pub fn snapshot_name(path: &Path) -> String {
    relative_to_manifest(path)
        .iter()
        .map(|component| component.to_string_lossy())
        .collect::<Vec<_>>()
        .join("__")
        .replace('.', "_")
}

pub fn collect_snapshot(path: &Path) -> Snapshot {
    let display = path.display();
    let mut sas =
        SasReader::open(path).unwrap_or_else(|err| panic!("failed to open {}: {}", display, err));
    let metadata = sas.metadata().clone();
    let columns = metadata
        .variables
        .iter()
        .map(|var| var.name.trim_end().to_string())
        .collect();

    let mut rows_iter = sas
        .rows()
        .unwrap_or_else(|err| panic!("failed to create row iterator for {}: {}", display, err));
    let mut rows = Vec::new();
    while let Some(row) = rows_iter
        .try_next()
        .unwrap_or_else(|err| panic!("error reading row from {}: {}", display, err))
    {
        let json_row = row.iter().map(common::value_to_json).collect();
        rows.push(json_row);
    }

    Snapshot {
        columns,
        row_count: metadata.row_count as usize,
        rows,
    }
}

static READSTAT_AVAILABLE: OnceLock<bool> = OnceLock::new();
static READSTAT_TEMP_COUNTER: AtomicUsize = AtomicUsize::new(0);
static CPP_AVAILABLE: OnceLock<bool> = OnceLock::new();
static CSHARP_AVAILABLE: OnceLock<bool> = OnceLock::new();
static CPP_TEMP_COUNTER: AtomicUsize = AtomicUsize::new(0);
static CSHARP_TEMP_COUNTER: AtomicUsize = AtomicUsize::new(0);

fn readstat_bin() -> String {
    std::env::var("SAS7BDAT_READSTAT_BIN").unwrap_or_else(|_| "readstat".to_string())
}

pub fn readstat_available() -> bool {
    *READSTAT_AVAILABLE.get_or_init(|| {
        let bin = readstat_bin();
        Command::new(bin).arg("--version").output().is_ok()
    })
}

pub fn collect_readstat_snapshot(path: &Path) -> Option<Snapshot> {
    let display = path.display();
    let display_str = display.to_string();
    let mut file =
        fs::File::open(path).unwrap_or_else(|err| panic!("failed to open {}: {}", display, err));
    let parsed = parse_metadata(&mut file)
        .unwrap_or_else(|err| panic!("failed to parse metadata for {}: {}", display, err));
    let column_kinds: Vec<ColumnKind> = parsed.columns.iter().map(|col| col.kind).collect();

    let temp_path = readstat_temp_path();
    let output = Command::new(readstat_bin())
        .arg(path)
        .arg(&temp_path)
        .output()
        .unwrap_or_else(|err| panic!("failed to run readstat for {}: {}", display, err));
    if !output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if stderr.to_lowercase().contains("unsupported character set") {
            return None;
        }
        panic!(
            "readstat failed for {}: status {:?}\nstdout: {}\nstderr: {}",
            display, output.status, stdout, stderr
        );
    }

    if !temp_path.exists() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if stderr.to_lowercase().contains("unsupported character set") {
            return None;
        }
        panic!(
            "readstat did not produce output for {}: expected {}\nstdout: {}\nstderr: {}",
            display,
            temp_path.display(),
            stdout,
            stderr
        );
    }

    let snapshot = collect_csv_snapshot(&temp_path, &column_kinds, &display_str, "readstat");

    let _ = fs::remove_file(&temp_path);

    Some(snapshot)
}

pub fn cpp_available() -> bool {
    *CPP_AVAILABLE.get_or_init(|| {
        let script = common::repo_root()
            .join("benchmarks")
            .join("runners")
            .join("run_cpp.sh");
        if !script.exists() {
            return false;
        }
        let fixture = common::fixture_path("fixtures/raw_data/pandas/airline.sas7bdat");
        Command::new(script)
            .arg("--build-only")
            .arg(&fixture)
            .output()
            .map(|output| output.status.success())
            .unwrap_or(false)
    })
}

pub fn csharp_available() -> bool {
    *CSHARP_AVAILABLE.get_or_init(|| {
        let script = common::repo_root()
            .join("benchmarks")
            .join("runners")
            .join("run_csharp.sh");
        if !script.exists() {
            return false;
        }
        let fixture = common::fixture_path("fixtures/raw_data/pandas/airline.sas7bdat");
        Command::new(script)
            .arg("--build-only")
            .arg(&fixture)
            .output()
            .map(|output| output.status.success())
            .unwrap_or(false)
    })
}

pub fn collect_cpp_snapshot(path: &Path) -> Option<Snapshot> {
    let display = path.display();
    let display_str = display.to_string();
    let mut file =
        fs::File::open(path).unwrap_or_else(|err| panic!("failed to open {}: {}", display, err));
    let parsed = parse_metadata(&mut file)
        .unwrap_or_else(|err| panic!("failed to parse metadata for {}: {}", display, err));
    let column_kinds: Vec<ColumnKind> = parsed.columns.iter().map(|col| col.kind).collect();

    let temp_path = cpp_temp_path();
    let script = common::repo_root()
        .join("benchmarks")
        .join("runners")
        .join("run_cpp.sh");
    let output = Command::new(script)
        .arg("--csv-output")
        .arg(&temp_path)
        .arg(path)
        .output()
        .unwrap_or_else(|err| panic!("failed to run cpp benchmark for {}: {}", display, err));
    if !output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if is_unsupported_charset(&stderr) {
            return None;
        }
        panic!(
            "cpp benchmark failed for {}: status {:?}\nstdout: {}\nstderr: {}",
            display, output.status, stdout, stderr
        );
    }

    if !temp_path.exists() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if is_unsupported_charset(&stderr) {
            return None;
        }
        panic!(
            "cpp benchmark did not produce output for {}: expected {}\nstdout: {}\nstderr: {}",
            display,
            temp_path.display(),
            stdout,
            stderr
        );
    }

    let snapshot = collect_csv_snapshot(&temp_path, &column_kinds, &display_str, "cpp");
    let _ = fs::remove_file(&temp_path);
    Some(snapshot)
}

pub fn collect_csharp_snapshot(path: &Path) -> Option<Snapshot> {
    let display = path.display();
    let display_str = display.to_string();
    let mut file =
        fs::File::open(path).unwrap_or_else(|err| panic!("failed to open {}: {}", display, err));
    let parsed = parse_metadata(&mut file)
        .unwrap_or_else(|err| panic!("failed to parse metadata for {}: {}", display, err));
    let column_kinds: Vec<ColumnKind> = parsed.columns.iter().map(|col| col.kind).collect();

    let temp_path = csharp_temp_path();
    let script = common::repo_root()
        .join("benchmarks")
        .join("runners")
        .join("run_csharp.sh");
    let output = Command::new(script)
        .arg("--csv-output")
        .arg(&temp_path)
        .arg(path)
        .output()
        .unwrap_or_else(|err| panic!("failed to run csharp benchmark for {}: {}", display, err));
    if !output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if is_unsupported_charset(&stderr) {
            return None;
        }
        panic!(
            "csharp benchmark failed for {}: status {:?}\nstdout: {}\nstderr: {}",
            display, output.status, stdout, stderr
        );
    }

    if !temp_path.exists() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        if is_unsupported_charset(&stderr) {
            return None;
        }
        panic!(
            "csharp benchmark did not produce output for {}: expected {}\nstdout: {}\nstderr: {}",
            display,
            temp_path.display(),
            stdout,
            stderr
        );
    }

    let snapshot = collect_csv_snapshot(&temp_path, &column_kinds, &display_str, "csharp");
    let _ = fs::remove_file(&temp_path);
    Some(snapshot)
}

fn readstat_temp_path() -> PathBuf {
    let count = READSTAT_TEMP_COUNTER.fetch_add(1, Ordering::Relaxed);
    let mut path = std::env::temp_dir();
    path.push(format!(
        "sas7bdat-readstat-{}-{}.csv",
        std::process::id(),
        count
    ));
    path
}

fn cpp_temp_path() -> PathBuf {
    let count = CPP_TEMP_COUNTER.fetch_add(1, Ordering::Relaxed);
    let mut path = std::env::temp_dir();
    path.push(format!("sas7bdat-cpp-{}-{}.csv", std::process::id(), count));
    path
}

fn csharp_temp_path() -> PathBuf {
    let count = CSHARP_TEMP_COUNTER.fetch_add(1, Ordering::Relaxed);
    let mut path = std::env::temp_dir();
    path.push(format!(
        "sas7bdat-csharp-{}-{}.csv",
        std::process::id(),
        count
    ));
    path
}

fn collect_csv_snapshot(
    csv_path: &Path,
    column_kinds: &[ColumnKind],
    display: &str,
    label: &str,
) -> Snapshot {
    let csv_file = fs::File::open(csv_path).unwrap_or_else(|err| {
        panic!(
            "failed to read {label} output for {} at {}: {}",
            display,
            csv_path.display(),
            err
        )
    });
    let mut reader = ReaderBuilder::new().has_headers(true).from_reader(csv_file);
    let headers = reader
        .headers()
        .unwrap_or_else(|err| panic!("failed to read headers for {}: {}", display, err))
        .iter()
        .map(|name| name.trim_end().to_string())
        .collect::<Vec<_>>();

    let mut rows = Vec::new();
    for record in reader.records() {
        let record =
            record.unwrap_or_else(|err| panic!("failed to read row for {}: {}", display, err));
        let mut row = Vec::with_capacity(record.len());
        for (idx, field) in record.iter().enumerate() {
            let kind = column_kinds
                .get(idx)
                .unwrap_or_else(|| panic!("missing column kind {} for {}", idx, display));
            row.push(csv_field_to_json(field, *kind, display, idx));
        }
        rows.push(row);
    }

    Snapshot {
        columns: headers,
        row_count: rows.len(),
        rows,
    }
}

fn csv_field_to_json(
    field: &str,
    kind: ColumnKind,
    display: &str,
    index: usize,
) -> serde_json::Value {
    if field.is_empty() || field == "." {
        return match kind {
            ColumnKind::Character => json!({ "kind": "string", "value": "" }),
            ColumnKind::Numeric(_) => json!({ "kind": "missing", "value": null }),
        };
    }

    match kind {
        ColumnKind::Character => json!({ "kind": "string", "value": field }),
        ColumnKind::Numeric(numeric_kind) => {
            let parsed = field.parse::<f64>().unwrap_or_else(|err| {
                panic!(
                    "failed to parse numeric field '{}' (column {}) for {}: {}",
                    field, index, display, err
                )
            });
            let kind_label = match numeric_kind {
                NumericKind::Double => "number",
                NumericKind::Date => "date",
                NumericKind::DateTime => "datetime",
                NumericKind::Time => "time",
            };
            json!({ "kind": kind_label, "value": parsed })
        }
    }
}

fn is_unsupported_charset(stderr: &str) -> bool {
    let lower = stderr.to_lowercase();
    lower.contains("unsupported character set")
        || lower.contains("unsupported charset")
        || (lower.contains("unsupported") && lower.contains("encoding"))
        || lower.contains("unsupported code page")
        || lower.contains("unknown code page")
}
