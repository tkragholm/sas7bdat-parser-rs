...
use super::runtime_column::RuntimeColumnRef;
...
pub struct ColumnMajorColumn {
    pub(crate) column: RuntimeColumnRef,
    buffer: Vec<u8>,
}
...
impl ColumnMajorBatch<'_> {
    ...
    #[must_use]
    pub fn column(&self, index: usize) -> Option<ColumnMajorColumnView<'_>> {
        let column = self.columns.get(index)?;
...
impl ColumnMajorColumn {
    pub const fn new(column: RuntimeColumnRef) -> Self {
        Self {
            column,
            buffer: Vec::new(),
        }
    ...
    pub fn prepare_rows(&mut self, rows: usize) {
    ...
    pub fn write_cell(&mut self, row_index: usize, value: &[u8]) {
        let width = self.column.width;
